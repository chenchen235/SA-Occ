...
class ASPP(nn.Module):
    def __init__(self, inplanes, mid_channels=256, outplanes=256, BatchNorm=nn.BatchNorm2d, dilations = [1, 6, 12, 18]):
        ...
        self.conv1 = nn.Conv2d(
            int(mid_channels * 5), outplanes, 1, bias=False)
...
class DepthNet(nn.Module):
    def __init__(self,
        ...
        depth_channels_ = depth_channels - 2
        if stereo:
            depth_conv_input_channels = depth_conv_input_channels + depth_channels_
            downsample = nn.Conv2d(depth_conv_input_channels,
                                    mid_channels, 1, 1, 0)
            cost_volumn_net = []
            for stage in range(int(2)):
                cost_volumn_net.extend([
                    nn.Conv2d(depth_channels_, depth_channels_, kernel_size=3,
                              stride=2, padding=1),
                    nn.BatchNorm2d(depth_channels_)])
        ...
        if use_aspp:
            ...
            depth_conv_list.append(ASPP(mid_channels, aspp_mid_channels, aspp_mid_channels))
        if use_dcn:
...
class SAT_DepthNet(nn.Module):
    def __init__(self,
                 ...
                 bias=0.0,
                 aspp_mid_channels=-1):
        super(SAT_DepthNet, self).__init__()
        self.reduce_conv = nn.Sequential(
            nn.Conv2d(
                in_channels, mid_channels, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(mid_channels),
        ...
        if use_aspp:
            if aspp_mid_channels < 0:
                aspp_mid_channels = mid_channels
                out_channels = mid_channels
            depth_conv_list.append(ASPP(mid_channels, aspp_mid_channels, out_channels))
        if use_dcn:
            depth_conv_list.append(
                build_conv_layer(
                    cfg=dict(
    ...
    def forward(self, x):
        ...
        if self.with_cp:
            ...
            # 3*res blocks +ASPP/DCN + Conv(c_mid-->D)
            depth = self.depth_conv(x)  # x: (B*N_views, C_mid, fH, fW) --> (B*N_views, D, fH, fW)
        return torch.cat([depth, context], dim=1)