...
class SA_OCC_Stereo4D(SA_OCC_4D):
    def __init__(self, **kwargs):
        super(SA_OCC_Stereo4D, self).__init__(**kwargs)
    ...
    def prepare_bev_feat(self, img, sensor2keyego, ego2global, intrin,
                         ...
                         k2s_sensor, extra_ref_frame, img_sat):
        """
        Args:
            img:  (B, N_views, 3, H, W)
        ...
        if extra_ref_frame:
            ...
            return None, None, None, None, None, None, None, stereo_feat
        # x: (B, N_views, C, fH, fW)
        # stereo_feat: (B*N, C_stereo, fH_stereo, fW_stereo)
        x, stereo_feat, x_sat, img_sat = self.image_encoder(img, img_sat, stereo=True)
        ...
        bev_feat, depth, bev_mask, semantic, sat_sem, sat_height = self.img_view_transformer(
            [x, sensor2keyego, ego2global, intrin, post_rot, post_tran, bda,
             mlp_input, x_sat], metas)
        if self.pre_process:
            bev_feat = self.pre_process_net(bev_feat)[0]    # (B, C, Dy, Dx)
        return bev_feat, depth, bev_mask, semantic, sat_sem, sat_height, img_sat, stereo_feat
    ...
    def extract_img_feat(self,
        ...
        bda, curr2adjsensor, img_sats = self.prepare_inputs(img_inputs, stereo=True)
        ...
        for fid in range(self.num_frame-1, -1, -1):
            ...
            if key_frame or self.with_prev:
                ...
                inputs_curr = (img, sensor2keyego, ego2global, intrin,
                               ...
                               extra_ref_frame, img_sat)

                if key_frame:
                    bev_feat, depth, bev_mask, semantic, sat_sem, sat_height, img_sat, feat_curr_iv = \
                        self.prepare_bev_feat(*inputs_curr)
                else:
                    with torch.no_grad():
                        bev_feat, _, _, _, _, _, _, feat_curr_iv = \
                            self.prepare_bev_feat(*inputs_curr)
