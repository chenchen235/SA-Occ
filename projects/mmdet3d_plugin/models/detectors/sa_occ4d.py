...
class SA_OCC_4D(SA_OCC):
    ...
    def __init__(self,
                 ...
                 with_prev=True,
                 **kwargs):
        super(SA_OCC_4D, self).__init__(**kwargs)
    ...
    def prepare_bev_feat(self, img, sensor2egos, ego2globals, intrin, post_rot, post_tran,
        ...
        Returns:
            ...
            depth: (B*N, D, fH, fW)
        """
        x, _, x_sat, img_sat = self.image_encoder(img, sat_img)      # x: (B, N, C, fH, fW)
        # bev_feat: (B, C * Dz(=1), Dy, Dx)
        # depth: (B * N, D, fH, fW)
        bev_feat, depth, bev_mask, semantic, sat_sem, sat_height = self.img_view_transformer(
            [x, sensor2egos, ego2globals, intrin, post_rot, post_tran, bda, mlp_input, x_sat])

        if self.pre_process:
            bev_feat = self.pre_process_net(bev_feat)[0]    # (B, C, Dy, Dx)
        return bev_feat, depth, bev_mask, semantic, sat_sem, sat_height, img_sat
    ...
    def extract_img_feat_sequential(self, inputs, feat_prev):
        ...
        sensor2keyegos_prev, _, post_rots, post_trans, bda = inputs[4:8]
        sat_img = inputs[8]
        bev_feat_list = []
        ...
        inputs_curr = (imgs, sensor2keyegos_curr[0:1, ...],
                       ...
                       post_trans, bda[0:1, ...], mlp_input, sat_img)

        # (1, C, Dx, Dy), (1*N, D, fH, fW)
        bev_feat, depth, bev_mask, semantic, sat_sem, sat_height, img_sat = self.prepare_bev_feat(*inputs_curr)
        ...
        return [x], depth, bev_mask, semantic, sat_sem, sat_height, img_sat
    ...
    def prepare_inputs(self, img_inputs, stereo=False):
        ...
        Returns:
            ...
            bda: (B, 3, 3)
        """
        # print(img_inputs[0].shape, img_inputs[7].shape)
        ...
        B, _, C, H, W = img_inputs[7].shape
        # if stereo:
            # img_sat = img_inputs[7].view(B, self.num_frame-1, C, H, W)
            # print(img_sat.shape)
        # else:    
        img_sat = img_inputs[7].view(B, self.num_frame, C, H, W)    # (B, N_views, N_frames, C, H, W)
        img_sat = torch.split(img_sat, 1, 1)
        img_sat = [t.squeeze(1) for t in img_sat]     # List[(B, N_views, C, H, W), (B, N_views, C, H, W), ...]
        # print(len(img_sat), img_sat[0].shape)
    ...
    def extract_img_feat(self,
        ...
        bda, _, img_sats = self.prepare_inputs(img_inputs)
        ...
        for img, sensor2keyego, ego2global, intrin, post_rot, post_tran, img_sat in zip(
                imgs, sensor2keyegos, ego2globals, intrins, post_rots, post_trans, img_sats):
            if key_frame or self.with_prev:
                ...
                inputs_curr = (img, sensor2keyego, ego2global, intrin, post_rot,
                               post_tran, bda, mlp_input, img_sat)
                if key_frame:
                    # bev_feat: (B, C, Dy, Dx)
                    # depth: (B*N_views, D, fH, fW)
                    bev_feat, depth, bev_mask, semantic, sat_sem, sat_height, img_sat = self.prepare_bev_feat(*inputs_curr)
        ...
        x = self.bev_encoder(bev_feat)
        # print(depth_list[0].shape, bev_mask[0].shape, sat_sem[0].shape, sat_height[0].shape)
        return [x], depth_list[0], bev_mask_list[0], semantic_list[0], sat_sem_list[0], sat_height_list[0], img_sat
